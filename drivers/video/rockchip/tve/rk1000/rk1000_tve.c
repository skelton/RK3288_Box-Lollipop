...
{	
	...
	if(gpio_is_valid(rk1000_tve.io_switch.gpio)) {
		if(tv_mode < TVOUT_YPbPr_720x480p_60)
			gpio_direction_output(rk1000_tve.io_switch.gpio, !(rk1000_tve.io_switch.active));
		else
			gpio_direction_output(rk1000_tve.io_switch.gpio, rk1000_tve.io_switch.active);
	}
	return 0;
...
static void rk1000_early_suspend(void *h)
{
	...
	if(rk1000_tve.ypbpr) {
		rk1000_tve.ypbpr->ddev->ops->setenable(rk1000_tve.ypbpr->ddev, 0);
		rk1000_tve.ypbpr->suspend = 1;
	}
	if(rk1000_tve.cvbs) {
		rk1000_tve.cvbs->ddev->ops->setenable(rk1000_tve.cvbs->ddev, 0);
...
static int rk1000_fb_event_notify(struct notifier_block *self, unsigned long action, void *data)
{
...
static int rk1000_tve_probe(struct i2c_client *client,const struct i2c_device_id *id)
{
	...
	rk1000_tve.io_switch.gpio = of_get_named_gpio_flags(tve_np,"gpio-reset", 0, &flags);
	if (gpio_is_valid(rk1000_tve.io_switch.gpio)) {
		rc = gpio_request(rk1000_tve.io_switch.gpio, "rk1000-tve-swicth-io");
		if(!rc){
			rk1000_tve.io_switch.active = !(flags & OF_GPIO_ACTIVE_LOW);
			gpio_direction_output(rk1000_tve.io_switch.gpio, !(rk1000_tve.io_switch.active));
		}
		else
			printk("gpio_request rk1000-tve-swicth-io invalid: %d\n",rk1000_tve.io_switch.gpio);
	}
	...
	printk("video src is lcdc%d, prop is %d\n", rk1000_tve.video_source, rk1000_tve.property);
#endif	
	...
	printk(KERN_INFO "rk1000 tvencoder ver 2.0 probe ok\n");
 	return 0;
failout:
	if(client)
		kfree(client);
